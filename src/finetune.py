"""
Fine-tuning DistilBERT on IMDB sentiment.

Uses the same tokenizer setup as Day 1 (distilbert-base-uncased).
"""
...
def load_imdb(path: Path = DATA_PATH) -> tuple[list[str], list[int]]:
    df = pd.read_csv(path)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="Fraction of IMDB to use (default: 0.25 = 25%%)",
    )
    parser.add_argument(
        "--max-samples",
        type=int,
    ...
    if args.max_samples is not None:
        target_n = min(args.max_samples, n_full)
    else:
        target_n = max(2, int(n_full * args.fraction))

    if target_n < n_full:
        df = pd.DataFrame({"text": texts, "label": labels})
        n_per = max(1, target_n // 2)
        parts = [
            g.sample(n=min(len(g), n_per), random_state=42)
            for _, g in df.groupby("label")
        ]
        df = pd.concat(parts).sample(frac=1.0, random_state=42).reset_index(drop=True)
        texts, labels = df["text"].tolist(), df["label"].tolist()
        print(f"Using subset: {len(texts)} / {n_full} samples ({100 * len(texts) / n_full:.1f}%)")
    else:
        print(f"Using full dataset: {n_full} samples")
    ...
    RESULTS_PATH.write_text(
        f"model: {MODEL_NAME}\n"
        f"n_samples: {len(texts)} (of full IMDB)\n"
        f"fraction/max_samples: fraction={args.fraction}, max_samples={args.max_samples}\n"
        f"train/val: 80/20, stratify, random_state=42\n"
